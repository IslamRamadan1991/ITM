# Copyright (C) 2021 TREVI Software
# License AGPL-3.0 or later (http://www.gnu.org/licenses/agpl).


# flake8: noqa
from . import access
from . import application
from . import application_license
from . import backup
from . import equipment
from . import equipment_component
from . import equipment_configuration
from . import equipment_db
from . import equipment_dbsetting
from . import equipment_forward
from . import equipment_function
from . import equipment_ipreservation
from . import equipment_mapping
from . import equipment_network
from . import equipment_partition
from . import equipment_rule
from . import equipment_type
from . import equipment_worklog
from . import partner
from . import service_ad
from . import service_dhcp
from . import service_proxy
from . import service_vpn
from . import service_wireless
from . import site
from . import services_password
from . import equipment_maintaince

